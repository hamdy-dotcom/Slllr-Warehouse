...
/**
 * The approved requests a release can be booked against, per product. A
 * release must name one — the RPC refuses without it.
 */
...
> {
  ...
  const { data, error } = await supabase
    ...
    .select("id, qty_approved, created_at, note, products!inner(sku)")
    .eq("status", "approved")
    .order("created_at", { ascending: true })
    .overrideTypes<(ApprovedRequestOption & { products: { sku: string } })[]>();
